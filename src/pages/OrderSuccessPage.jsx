  ...
  return (
    <div className="bg-cream min-h-screen">
      <div className="mx-auto max-w-4xl px-4 py-10">
        ...
        {/* Success hero */}
        <div className="mb-8 rounded-2xl bg-white px-6 py-8 text-center shadow-sm">
          <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-soft-green animate-[scale-in_0.4s_ease-out]">
            <CheckCircle2 size={34} className="text-primary" />
          </div>
          <h1 className="text-2xl font-bold text-primary-dark">Yêu cầu đặt hàng đã được ghi nhận</h1>
          <p className="mt-2 text-sm text-secondary max-w-md mx-auto">
            Cảm ơn bạn đã gửi yêu cầu đặt hàng tại Merifarm. Đội ngũ Merifarm sẽ liên hệ để xác nhận sản phẩm, phí vận chuyển và thời gian giao hàng dự kiến.
          </p>
          <div className="mt-4 inline-flex items-center gap-2 rounded-full bg-gold-soft px-5 py-2 text-sm font-bold text-accent-dark">
            <FileText size={15} />
            Mã yêu cầu: {code}
          </div>
        </div>
                ...
                <div key={step.label} className="flex flex-1 flex-col items-center">
                  <div className="flex w-full items-center">
                    ...
                    <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full border-2 transition-all duration-300
                      ${active ? 'border-primary bg-primary shadow-md' : done ? 'border-primary bg-primary' : 'border-soft bg-white'}`}>
                      <Icon size={16} className={active || done ? 'text-white' : 'text-faint'} />
                    </div>
                    {!last && (
                ...
                <div key={step.label} className="flex items-start gap-3">
                  <div className="flex flex-col items-center">
                    <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full border-2 transition-colors
                      ${active ? 'border-primary bg-primary' : done ? 'border-primary bg-primary' : 'border-soft bg-white'}`}>
                      <Icon size={14} className={active || done ? 'text-white' : 'text-faint'} />
                    </div>
            ...
            <div className="divide-y divide-soft">
              {items.map((item) => (
                <div key={item.id} className="flex gap-3 p-4">
                  <img src={item.image} alt={item.name} className="h-14 w-14 rounded-lg object-cover ring-1 ring-soft shrink-0" />
          ...
          <div className="rounded-2xl bg-white shadow-sm overflow-hidden">
            ...
            <ul className="divide-y divide-soft">
              {[
                ...
                'Liên hệ xác nhận đơn hàng qua số điện thoại đã cung cấp.',
                'Thông báo phí vận chuyển chính xác nếu có thay đổi.',
                'Tiến hành chuẩn bị và giao hàng sau khi đơn được xác nhận.',
              ].map((step, i) => (
                <li key={i} className="flex items-start gap-3 px-5 py-3.5">
                  <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-soft-green text-[11px] font-bold text-primary">
                    {i + 1}
                  </span>
